    ...
    if (!empty($login) && !empty($senha)) {
        try {
            ...
            if ($user && $senha === $user['senha']) { // Em produção, usar password_verify()
                ...
                // Se a senha não foi alterada, redirecionar para a página de alteração de senha.
                // A lógica de bloqueio de 3 dias está no arquivo check_password_change.php.
                if ($user['senha_alterada'] == 0) {
                    // Redirecionar para a página de alteração de senha (que fará a verificação de 3 dias)
                    header('Location: includes/check_password_change.php');
                    exit();
                }
