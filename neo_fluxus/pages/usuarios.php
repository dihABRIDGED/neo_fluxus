    ...
    try {
        switch ($_POST['action']) {
            case 'create':
                ...
                // Se for aluno, auto-matricular em todas as disciplinas ativas
                if ($tipo === 'aluno') {
                    // Buscar todas as disciplinas ativas
                    $stmt_disciplinas = $pdo->prepare("SELECT id FROM disciplina WHERE ativo = 1");
                    $stmt_disciplinas->execute();
                    $disciplinas_ativas = $stmt_disciplinas->fetchAll(PDO::FETCH_ASSOC);
                    
                    // Matricular o aluno em cada disciplina ativa
                    $stmt_matricula = $pdo->prepare("INSERT INTO matricula (aluno_id, disciplina_id, ativo) VALUES (?, ?, 1)");
                    foreach ($disciplinas_ativas as $disciplina) {
                        $stmt_matricula->execute([$novo_usuario_id, $disciplina['id']]);
                    }
                    
                    if (count($disciplinas_ativas) > 0) {
                        $message .= ' O aluno foi automaticamente matriculado em ' . count($disciplinas_ativas) . ' disciplina(s) ativa(s).';
                    }
                }
