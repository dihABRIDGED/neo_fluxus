...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action"]) && $_POST["action"] === "register_attendance") {
    ...
    try {
        ...
        // 2. Get ALL students matriculated in the discipline
        $alunos_da_disciplina = [];
        $sql_alunos_disciplina_para_aula = "
            SELECT 
                u.id 
            FROM usuario u 
    ...
    // Get ALL students if disciplina_id is provided
    $selected_disciplina_id = $_GET["disciplina_id"] ?? null;
    $alunos_disciplina = [];
    if ($selected_disciplina_id) {
        // Get ALL students matriculated in the selected discipline
        $sql_alunos_disciplina = "
            SELECT 
                u.id, 
                u.nome 
            FROM usuario u 
